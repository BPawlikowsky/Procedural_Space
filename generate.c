...
starSystem generateSystem(ivec2d pos, char starType, int starSize,
                          int planetCount, int planetSize, SDL_Texture *tex) {
  ...
  for (int i = 0; i < tileSys.planetCount; i++) {
    ...
    int l =
        (generateRand(75, (int)(TEX_SIZE / 2) - planetSize * 10, nState + i));
    l += i * 10;
    int x = l * (cos(l)) + TEX_SIZE / 2.0f;
...
void initCircle(circle *c, float mass, int orbit, circle sun) {
  ...
  c->r = c->mass / 2;
  c->orbit = orbit;
  c->pos.x = TEX_SIZE / 2.0f;
  c->pos.y = (TEX_SIZE / 2.0f) - c->orbit;
...
float testOrbit(circle c, circle sun, double delta) {
  ...
  while (!done) {
    ...
    for (long int t = 0; t < time; t++) {
      ...
      float th = 10;
      if (dist > (TEX_SIZE / 2.0f) || dist > (test.orbit + th) ||
          dist < (test.orbit - th)) {
        val += 0.01;
        break;
      } else if (t > (time - 10)) {
