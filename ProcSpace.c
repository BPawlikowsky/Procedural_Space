...
circle sun = {{(TEX_SIZE / 2), (TEX_SIZE / 2)},
              30,
              5000,
              0,
              {0, 0},
...
void loop(void *_a) {
  args *a = _a;
  SDL_Event event;
  printf("done: %d, frames: %d, milis: %f\n", a->done, a->frames, a->milis);
...
#ifdef __EMSCRIPTEN__
    emscripten_set_main_loop_arg(loop, a, 0, 1);
#else
    ...
    for (int y = 0; y < STAR_MAP_SIZE; y++) {
      ...
      systems[x][y].texture =
          SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                            SDL_TEXTUREACCESS_STREAMING, TEX_SIZE, TEX_SIZE);
...
void render() {
  ...
  float scale = 0.007;
  SDL_RenderDrawPoint(renderer, (player.vel.x * scale * zoom) + 400,
                      (player.vel.y * scale * zoom) + 300);
  // Render buffer
  SDL_RenderPresent(renderer);
}

float zoomCalc() {
  return floatRange(0, 100, nMaxZoom, nMinZoom,
                    (player.vel.x > player.vel.y) ? player.vel.x
                                                  : player.vel.y);
}
